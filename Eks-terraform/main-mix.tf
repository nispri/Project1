...
# Create EKS Node Group with Mixed Instances Policy
resource "aws_eks_node_group" "KES" {
  ...
  mixed_instances_policy {
    ...
    launch_template {
      launch_template_name = aws_launch_template.KES.name
      version              = "$Latest"
    }
  }

  # Define that this node group should use Spot Instances with fallback to On-Demand
  capacity_type = "SPOT"
